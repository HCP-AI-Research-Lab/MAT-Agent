...
class DynamicCocoDetection(CocoDetection):
    ...
    def __getitem__(self, index):
        img, target, is_neg, original_img = super().__getitem__(index)
        img = Image.fromarray(np.uint8(img))
        img = self.dynamic_aug(img, self.current_epoch)
        if not isinstance(img, torch.Tensor):
            img = transforms.ToTensor()(img)
        target = target.to(torch.float32)
        return img, target, is_neg, original_img
...
def main():
    ...
    parser.add_argument('--data', type=str, default='./datasets/MSCOCO-2014', help='Path to COCO dataset')
...
def validate_multi(val_loader, model, ema_model):
    ...
    with torch.no_grad():
        for inputs, target, _, _ in val_loader:
            outputs = sigmoid(ema_model(inputs.to(next(model.parameters()).device)))
            preds.append(outputs.cpu())
            targets.append(target.max(dim=1)[0].cpu())

    return mAP(torch.cat(targets).numpy(), torch.cat(preds).numpy())
