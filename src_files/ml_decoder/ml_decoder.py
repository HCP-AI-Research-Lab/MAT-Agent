...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
        ...
        print("embed_len_decoder: ", embed_len_decoder)
    ...
    def forward(self, x):
        # 处理不同维度的输入
        if len(x.shape) == 4:  # 处理CNN特征 [bs, channels, height, width]
            # 从空间特征转换为序列 [bs, height*width, channels]
            embedding_spatial = x.flatten(2).transpose(1, 2)
            # 投影到decoder_embedding维度
            embedding_spatial = self.embed_standart(embedding_spatial)
        elif len(x.shape) == 2:  # 处理CSV数据特征 [bs, features]
            # 投影到decoder_embedding维度，然后添加序列维度
            embedding_spatial = self.embed_standart(x)  # [bs, decoder_embedding]
            # 添加假的序列维度，使其成为[bs, 1, decoder_embedding]
            embedding_spatial = embedding_spatial.unsqueeze(1)
        else:  # 已经是3D特征 [bs, seq_len, features]
            # 投影到decoder_embedding维度
            embedding_spatial = self.embed_standart(x)
        
        # 应用ReLU激活
        embedding_spatial = torch.nn.functional.relu(embedding_spatial, inplace=True)
        ...
        # 查询嵌入扩展到批次大小
        tgt = query_embed.unsqueeze(1).expand(-1, bs, -1)  # [num_queries, batch, embed_dim]
        
        # Transformer解码器期望输入是[seq_len, batch, embed_dim]
        memory = embedding_spatial.transpose(0, 1)
